class NFA:
    ...
    def construct_nfa_from_file(self, lines):
        ...
        for index in range(len(accepting_states_line)):
            if index == 0:
                self.num_accepting_states = int(accepting_states_line[index][1])
            else:
                self.accepting_states.append(int(accepting_states_line[index][1]))
        
        self.startState = int(lines[3][2])
        
        
        for index in range(4, len(lines)):
            transition_func_line = lines[index].split(" ")
            
            starting_state = int(transition_func_line[0][1])
            transition_symbol = transition_func_line[1] 
            ending_state = int(transition_func_line[2][1])
...
class DFA:
    ...
    def print_dfa(self):
        ...
        print('q' + str(self.num_accepting_states) + " " + " ".join('q' + str(accepting_state) for accepting_state in self.accepting_states))
        print('>q' + str(self.start_state))
